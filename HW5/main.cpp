...
void init() {
	...
	vec4 eye;//(0, 2, 0, 1);
	vec4 ambient(1, 1, 0, 1);
	vec4 specular(0, 1, 0, 1);
	vec4 diffuse(1, 0, 1, 1);
	a->setMaterial(diffuse, ambient,specular, 0.5);
	cc->setMaterial(diffuse, ambient, specular, 0.5);
	Light light1(eye, ambient,specular,diffuse);
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("TsaiAlanHW5");  //open a window with the title "2D Graphics"
...
void display() {
	...
	a->drawSphere(s, lights);
	a->drawSquare();
	cc->drawCube(s, lights);
